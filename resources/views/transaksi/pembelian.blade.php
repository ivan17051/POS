...
<div class="container-fluid">
  <form action="{{route('barang_keluar.store')}}" method="POST" id="formBeli"> @csrf <div class="row">
    <div class="col-md-6"> <div class="card"> <div class="card-header card-header-tabs card-header-primary">
...
<div class="typeahead__field">
  ...
  <input class="form-control js-typeahead" name="q" id="querybarang" autocomplete="on" onclick="$('#searchbarang
  .js-typeahead').val('');" placeholder="Cari Barang">
</div>
</div>
</div> -->
<select class="form-control selectpicker" id="selectbarang" data-style="select-with-transition" data-live-search="true"
  onchange="pilihBarang(this)" data-size="5">
  <option value="" disabled>Cari Barang</option>
  @foreach($barang as $unit)
  <option value="{{$unit->id}}||{{$unit->harga_1}}||{{$unit->harga_3}}||{{$unit->harga_6}}||{{$unit->namabarang}}"
    data-stok="{{$unit->qty}}">{{$unit->kodebarang}} - {{$unit->namabarang}} [Stok {{$unit->qty}}]</option>
  @endforeach
  </select>
  </div>
  </div>
  ...
  <div class="form-group">
    ...
    <select class="selectpicker" name="metode" id="metode" data-style="select-with-transition" required
      onchange="show(this)">
  ...
  @if(isset($struk))
  <a id="cetakStruk" href="{{route('cetak.struk',['id'=>$struk])}}" onclick="window.open(this, '_blank');return false;"
    hidden></a>
  @endif
  </div>
  @endsection
  ...
  <script>
    ...
    function pilihBarang(self) {
      ...
      var stok = $('option:selected', self).attr('data-stok');
