            ...
            <tr>
                <td class="jarak">
                    ...
                    <table cellspacing="0" cellpadding="0" >
                        <tbody>
                            <tr>
                              <td class="headerFont fontCenter  fontUnderline" style="font-size:16px">LAPORAN PIH</td>
                            </tr>
                            <tr>
                    ...
                    <table class="w-90">
                      <thead class="tb-header">
                        <tr>
                          ...
                          <th class="headerFont fontCenter fontBold" style="font-size:13px; width:15%;">PROSES</th>
                        </tr>
                      </thead>
                      <tbody class="tb-body font-13">
                        @foreach($dataSip as $key=>$unit)
                        <tr>
                          ...
                          <td class=" fontCenter">{{$unit->proses}} Hari Kerja</td>
                        </tr>
                        @endforeach
                        @foreach($dataSurat as $key=>$unit)
                        <tr>
                          <td class=" fontJustify">{{count($dataSip)+key+1}}. </td>
                          <td class=" fontJustify">{{$unit->pegawai->nik}}</td>
                          <td class=" fontCenter">{{$unit->pegawai->nama}}</td>
                          <td class=" fontCenter">{{$unit->pegawai->profesi}}</td>
                          <td class=" fontCenter">{{$unit->nomor}}</td>
